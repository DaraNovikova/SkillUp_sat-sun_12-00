...
function validateFileType (str){
    console.log(str);
    return (/^(\w+\.){1,2}?(png|jpg|jpeg)$/g).test(str);
}
...
function checkYear(year){
    console.log(year);
    return (/19\d{2}$|20\d{2}$|2100/g).test(year);
}
...
function checkEmail (email){
    console.log(email);
    return (/^\w+(\.\w+|-\w+)?@(\w+\.){1,2}[a-z]{2,3}$/g).test(email)
}
 ...
 */
 ...
 function checkDomainUrl (url){
     console.log(url);
     return (/^(http|https):\/\/.+\.\w{2,4}$/g).test(url);
 }
...
function createLinksFromDomains(str) {
    str.match(/(?:http|https):\/\/(.+?(?:\.[a-z]+-[a-z]+){0,1}(?:\.\w{2,4}){1,2})/g).forEach((item) => str = str.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(http:|https:)\/\//gi, '')}</a>`));
    return str;
  }
