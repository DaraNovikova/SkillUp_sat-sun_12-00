...
function wordsList(str, subStr){
	str = String(str).replace(/[\.,!?;:"]/g, '').toLowerCase();
	list = str.split(" ");
	set = new Set();
...
function getLocalDate(date, isSeconds, isISO){
	...
	if (isSeconds === true && isISO ===true) {
		return date.toLocaleString().substr(0,20).replace(/(\d{2})\.(\d{2})\.(\d{4})/, "$3-$2-$1")
	} else if (isISO){
		return date.toLocaleString().substr(0,17).replace(/(\d{2})\.(\d{2})\.(\d{4})/, "$3-$2-$1")
	} else if (isSeconds){
		return date.toLocaleString().substr(0,20)
	}else {
		return date.toLocaleString().substr(0,17)
	}
